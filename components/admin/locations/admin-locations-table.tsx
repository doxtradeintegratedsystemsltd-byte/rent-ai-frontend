...
export default function AdminLocationsTable() {
  ...
  const user = useUser();
  const adminId = user?.id; // use the authenticated admin's user ID for filtering
            ...
            {tableData && tableData.length > 0 ? (
              tableData.map((row) => (
                <TableRow
                  ...
                  onClick={() => router.push(`/admin/location/${row.id}`)}
                >
              ...
              <TableNoData className="flex flex-col" colSpan={tableHead.length}>
                ...
                <p>Locations you create will appear here.</p>
              </TableNoData>
            )}
          </TableBody>
        </Table>
